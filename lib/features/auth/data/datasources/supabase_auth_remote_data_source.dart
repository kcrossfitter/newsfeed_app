...
class SupabaseAuthRemoteDataSource implements AuthRemoteDataSource {
  ...
  Future<User> signup({
    ...
    try {
      ...
      // Supabase에서 발생한 AuthException을 그대로 다시 throw 하거나,
      // 필요에 따라 커스텀 Exception (예: ServerException(message: e.message))으로 변환하여 throw 할 수 있습니다.
      // Repository 계층에서 이 Exception을 Failure로 변환합니다.
      // print('SupabaseAuthRemoteDataSource Error: ${e.message}'); // 디버깅용
      rethrow; // 우선 그대로 다시 던집니다.
    } catch (e) {
      // 예상치 못한 다른 종류의 에러 처리
  ...
  Future<User> login({required String email, required String password}) async {
    try {
      ...
      // 그대로 다시 throw 합니다. Repository 계층에서 Failure로 변환됩니다.
      rethrow;
    } catch (e) {
      // 기타 예상치 못한 에러 처리
      throw AuthException(
