...
9. update comment count trigger 
...
14. check admin status function 
...
15. profile access control policy 
-- SELECT Policy 
-- 정책 1: 사용자는 자신의 프로필을 조회할 수 있습니다. (최적화 적용) 
CREATE POLICY "Allow individual user to read their own profile" 
ON public.profiles 
FOR SELECT 
USING ((SELECT auth.uid()) = id); -- auth.uid() 호출 방식 변경 

-- 정책 2: 관리자는 모든 프로필을 조회할 수 있습니다. (최적화 적용) 
CREATE POLICY "Allow admin to read all profiles" 
ON public.profiles 
FOR SELECT 
USING (public.get_user_role((SELECT auth.uid())) = 'admin'); -- auth.uid() 호출 방식 변경 
...
16. newsfeed access control policy 
-- SELECT Policy 
...
CREATE POLICY "Allow admin to update newsfeeds" 
ON public.newsfeeds 
FOR UPDATE 
...
CREATE POLICY "Allow admin to delete newsfeeds" 
ON public.newsfeeds 
FOR DELETE 
